...
API_KEY = "4835381b54e89e33e148457aed03e39d"
UNSPLASH_ACCESS_KEY = "yUtVQTzps_QwoQMW881iHM_GB2ja6NNII4H-NxByBR4"
...
def apply_dynamic_css(dynamic_bg_style, accent_color):
    ...
    card_bg = "rgba(0, 0, 0, 0.2)" if is_dark else "rgba(255, 255, 255, 0.4)"
    ...
    sidebar_bg = "#0f1720" if is_dark else "#ffffff"
    sidebar_text = "#f0f2f6" if is_dark else "#333333"
    ...
    .stApp::before {{
        ...
        z-index: -1; filter: blur(8px) brightness({0.5 if is_dark else 1.0});
    }}
...
def render_hourly_forecast(city):
    ...
    if forecast_res and forecast_res.get("cod") == "200":
        hourly_data = forecast_res["list"][:8]
...
def display_app_content():
    ...
    with col_center_content:
        ...
        with rightin:
            # Hide detect location button for now
...
accent_color = "#4CAF50" if st.session_state.theme == 'dark' else "#6A5ACD"
